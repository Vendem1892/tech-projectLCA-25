    ...
    <div class="flex place-content-center">
        <div class="bg-white border-4 rounded-sm border-black shadow-md text-black h-50vh w-lg">
            ...
            <form action="includes/verifyingat.php" onsubmit="return loginValidation()" method="post" class="m-2">
                ...
                <input type="password" placeholder="Enter Password (8-20 characters)" name="pwd" id="pwd" minlength="8" maxlength="20" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" required class="w-full outline-2 focus:outline-none focus:ring-2 focus:ring-green-500">

                <br><br>

                <label for="pwd-rep" class="font-medium block text-base">Repeat Password</label><br>
                <input type="password" placeholder="Repeat Password" id="pwd-rep" name="pwd-rep" minlength="8" maxlength="20" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" required class=" w-full outline-2 focus:outline-none focus:ring-2 focus:ring-green-500">

                <br><br>

                <div>
                    <input type="submit" name="Login" value="Login" class="w-full flex justify-center border border-transparent rounded-sm py-2 px-4 bg-blue-500 hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-600 text-base text-center text-white"><br><br>
